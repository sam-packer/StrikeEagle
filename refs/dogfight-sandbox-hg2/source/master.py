...
class Main:
    ...
    # Threading event: network thread waits on this after requesting a scene
    # update in rendered mode, so the main thread finishes Main.update()
    # before the network thread processes the next command.
    import threading
    client_update_done = threading.Event()
    client_update_done.set()  # Initially signaled (no pending update)
