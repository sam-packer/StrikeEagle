...
import Physics
import traceback
from carrier_deck_data import build_api_carrier_deck_payload, build_local_carrier_deck_data
...
def init_server(main_):
	...
	commands_functions = {
		...
		"GET_CARRIER_DECK_PARAMETERS": get_carrier_deck_parameters,

		# Camera / view
		"SET_CAMERA_TRACK": set_camera_track,
		"SET_TRACK_VIEW": set_track_view
	}
	server_log = ""
			...
			while flag_server_connected:
				...
				if command == "":
					...
					try:
						...
						error_msg = "ERROR executing command '{}': {}\n{}".format(cmd_name, e, traceback.format_exc())
						print(error_msg)
						server_log += error_msg

		except Exception as e:
			error_msg = "network_server.py - server_update: {}\n{}".format(e, traceback.format_exc())
			print(error_msg)
			server_log += error_msg
			flag_server_connected = False
			main.flag_client_connected = False
...
def update_scene(args):
	if main.flag_client_update_mode:
		if main.flag_renderless:
			...
			# Rendered mode: tell main thread to update, then WAIT for it to
			# finish before returning. This prevents the network thread from
			# modifying game state while Main.update() is running.
...
def step_command(args):
	"""Combined command: apply controls, advance sim, return plane state.

	This is the fast path for RL training — one round-trip per env step instead
	of separate SET_PLANE_* + UPDATE_SCENE + GET_PLANE_STATE calls.
	...
	# Apply controls directly (skip individual command overhead)
...
def fire_missile(args):
	...
	if md is not None:
		...
		if target_id is not None:
			# Force target lock before firing so the missile gets guidance.
			# Without this, the missile launches unguided if target_locked is False.
			td = machine.get_device("TargettingDevice")
			if td is not None:
				td.set_target_by_name(target_id)
				td.target_locked = True
...
def get_runtime_carriers():
	return main.aircraft_carrier_allies + main.aircraft_carrier_ennemies


def get_carrier_by_name(carrier_id):
	for carrier in get_runtime_carriers():
		if carrier.name == carrier_id:
			return carrier
	return None
...
def get_carriers_list(args):
	carriers = []
	print("Get carriers list")
	for carrier in get_runtime_carriers():
		print(carrier.name)
		carriers.append(carrier.name)
	socket_lib.send_message(str.encode(json.dumps(carriers)))


def get_carrier_deck_parameters(args):
	carrier_id = args["carrier_id"]
	carrier = get_carrier_by_name(carrier_id)
	if carrier is None:
		socket_lib.send_message(str.encode(json.dumps({"error": "Carrier not found", "carrier_id": carrier_id})))
		return

	mat = carrier.get_parent_node().GetTransform().GetWorld()
	state = build_api_carrier_deck_payload(
		carrier.name,
		carrier.nationality,
		vec3_to_list(hg.GetT(mat)),
		{
			"x": vec3_to_list(hg.GetX(mat)),
			"y": vec3_to_list(hg.GetY(mat)),
			"z": vec3_to_list(hg.GetZ(mat)),
		},
		build_local_carrier_deck_data(),
	)
...
def get_missile_state(args):
	missile_id = args["missile_id"]
	if missile_id not in main.destroyables_items:
		state = {
			"timestamp": main.framecount,
			"timestep": main.timestep,
			"error": "missile_not_found",
			"destroyed": True,
			"wreck": False,
			"crashed": False,
			"active": False
	...
	machine = main.destroyables_items[missile_id]
	try:
		position = machine.get_position()
		rotation = machine.get_Euler()
		v_move = machine.get_move_vector()
		...
		print("WARNING: get_missile_state('{}') failed: {}".format(missile_id, e))
		state = {
			"timestamp": main.framecount,
			"timestep": main.timestep,
			"error": str(e),
...
def set_camera_track(args):
	"""Set the camera to follow a specific aircraft.

	Args:
		machine_id: plane name (e.g. "ally_1")
	"""
	machine_id = args["machine_id"]
	# Set the view carousel to this aircraft
	main.set_view_carousel("Aircraft_" + machine_id)
	# Also set user_aircraft so the HUD and audio listener follow
	player = main.get_player_from_caroursel_id("Aircraft_" + machine_id)
	if player is not None:
		main.user_aircraft = player
		player.set_focus()
	# Ensure SFX is enabled and initialized
	if not main.flag_sfx:
		main.set_activate_sfx(True)
	if flag_print_log:
		print("Camera tracking: " + machine_id)


def set_track_view(args):
	"""Set the camera angle relative to the tracked aircraft.

	Args:
		view: one of "back", "front", "left", "right", "top"
	"""
	view = args.get("view", "back")
	main.set_track_view(view)
	if flag_print_log:
		print("Track view: " + view)
